//--------------------------------------------------------------------------------
#define _FW_NAME     "KNXGATE_32"
#define _FW_VERSION  "VER_7.002 "
#define _ESP_CORE    "esp32-2.7.4"
...
//  WARNING - LA PUBBLICAZIONE AVVIENE SOLO SE PRIMA E' PERVENUTO UN /cmd qualunque da mqtt !!!!!!!!!!!!!!!
//        finchè non invia un comando che inizia con: '§'  

//-------------------------------------------------------------------------------------------------------------------------
// domotic_option 4 - KNX
// definire le cover con tipo 8 o 9 l'indirizzo base (DPTx_StopStepUpDown)) e con tipo 18 o 19 quello di move (DPTx_UpDown)
// le definizioni devono essere ordinate: ogni tapparella prima 8 e poi 18 o 9 e poi 19, ogni dimmer prima 4 e poi 24
// nella tabella posizioni vanno solamente gli indirizzi base (QUELLO PIU BASSO)
// dopo aver cercato in tabella l'indirizzo, se trovato e di tipo 18/19 riprende quello base dalla tabella
//-------------------------------------------------------------------------------------------------------------------------
//
//
//
...
/*
  ...
  V 2.0 - risponde anche a comandi HTTP, tipo:
          ...
          http://<ipaddress>:8080/json.htm?type=command&param=udevices&script=knxgate_json.lua&type=<byte[3]>&from=<byte[2]>&to=<byte[1]>&cmd=<byte[4]>
          è parametrizzabile la parte:
                            :8080/json.htm?type=command&param=udevices&script=knxgate_json.lua
                            (esempio conforme a DOMOTICZ e script  KNXGAtE_JSON.LUA
  ...
  chiamate http:
    ...
    server.on ("/request", handleRequest);          // mappa richiesta comandi knx
    server.on ("/gate.htm", handleGate);            // esecuzione comando knx
    server.on ("/gate", handleGate);                // esecuzione comando knx
    ...
    server.on ("/mqttdevices", handleMqttDevices);  // inizio processo di censimento automatico dei devices
...
char domotic_options;  // 1= knx - indirizzi base dispari   0= base pari   3=multi-address    4=random-address
...
String httpResp = "";   // resp= nessuna risposta      resp=i conferma immediata       resp=a risposta "get" con caratteri KNXGATE      resp=y  entrambe
...
#define E_MQTT_TABDEVICES 324	// mqtt_device max 128x5=640 -> 964
#define E_MQTT_TABLEN       5	// 
// ==============================================================================================================
...
// ==============================================================================================================
DEVADDR device_BUS_id[DEV_NR]; // index:device index,  contenuto: device address reale (knx) e tipo
char alexa_BUS_ix[DEV_NR];     // index:id alexa, contenuto: device index
// ==============================================================================================================
#define E_ALEXA_DESC_DEVICE 964 // device description max DEV_NR x E_ALEXA_DESC_LEN char 
...
void handleMqttDevices()  // inizio processo di censimento automatico dei devices scs
{
  ...
  if (request == "clear")
  {
    ...
    for (int i = 0; i < DEV_NR; ++i)
    {
      ...
      device_BUS_id[i].address = 0;
      device_BUS_id[i].deviceType = 0;
      device_BUS_id[i].alexa_id = 0;
      device_BUS_id[i].linesector = 0;
      device_BUS_id[i].moveaddress = 0;
    }
        ...
        while ((devx < DEV_NR) && (device_BUS_id[devx].addressW))
        {
          ...
          if ((devtype > 0) && (devtype < 32))
          {
            ...
            if ((devtype == 9) || (devtype == 19))  // tapparelle pct
            {
              ...
              {
                char hBuffer[32];
                //                                                        moveaddress     maxposition     position        direction       request         timeout
                sprintf(hBuffer, " %02X %02X %02X% 02X %02X %02X ", replyBuffer[1], replyBuffer[2], replyBuffer[3], replyBuffer[4], replyBuffer[5], replyBuffer[6]);
                content += hBuffer;
              }
              else
              {
...
//    else
  ...
  if (command != 0xFF)
  {
    ...
    if ((devtype == 9) || (devtype == 8) || (devtype == 18) || (devtype == 19))	// <====================COVERPCT=========================
    {
      requestBuffer[requestLen++] = '§';
      requestBuffer[requestLen++] = 'u';
...
void handleStatus()
{
  ...
  {
    Serial2.write('@');   // set led lamps
    Serial2.write(0xF2);  // set led lamps high-freq (AP mode)
    content += "<li>";
  ...
  {
    Serial2.write('@');   // set led lamps
    Serial2.write(0xF1);  // set led lamps std-freq (client mode)
    content += "<li>";
...
// @Qr
  Serial2.write('§');    // command mode 
  Serial2.write('Q');    // command: query 
  Serial2.write('r');    // query PIC fw version
  delay(50);            // wait 50ms
  char nrs = 0;
  nrs = Serial2.read();        // receive from Serial2 USB
  content += "<li>";
  content += "ESP resets: ";
...
void handlePicProg(void)          // verify / start PIC firmware programming
{
  ...
  if (sm_picprog == PICPROG_FREE)
  {
    Serial2.flush();
    Serial2.write('§');    // 
    Serial2.write('q');    // query PIC fw version
    delay(50);            // wait 50ms
    char sl = 0;
    if (Serial2.available() )
    {
      while (Serial2.available() && (sl < 15))
      {
        picfwVersion[sl++] = Serial2.read();        // receive from Serial2 USB
...
void immediateSend(void)
{
  if (requestLen > 0)
  {
    // =========================== send control char and data over Serial2 =============================
    String log = "\r\ntx: ";
    char logCh[4];
    uartSemaphor = 1;
    int s = 0;
    ...
    {
#ifdef UART_W_BUFFER
          Serial2.write(requestBuffer[s]);    //  scrittura Serial2E 
  #ifdef USE_TCPSERVER
          if (tcpuart == 2) 
          {
...
char immediateReceive(char firstChar)
{
  ...
  while ((!Serial2.available() ) && (wait < OUTERWAIT * 100))
  {
  ...
  if (Serial2.available() )
  {
    String log = "\r\nrx: ";
    char logCh[4];
    while (Serial2.available() && (replyLen < 255))
    {
      while (Serial2.available() && (replyLen < 255))
      {
        replyBuffer[replyLen] = Serial2.read();        // receive from serial USB
#ifdef MQTTLOG
        if (mqtt_log == 'y')
        {
...
//      delayMicroseconds(OUTERWAIT);
      ...
      wait = 0;
      while ((!Serial2.available() ) && (wait < OUTERWAIT / 5))
      {
        delayMicroseconds(5);
        wait++;
      }
...
// initialize uart and uart2
  ...
  Serial.begin(115200, SERIAL_8N2);		// debug monitor
  while (!Serial) {
    ;
  }
  Serial.flush();
  ...
  Serial2.begin(115200, SERIAL_8N2, 5,17);
  ...
  Serial2.write('@');    // 
  Serial2.write(0xF0);   // set led lamps low-freq (in progress mode)
  delay(10);            // wait 10ms
...
// -----------------------------------------------------------------------------------------------------------
  ...
  if (Serial2.available() )
  {
    while (Serial2.available() && (sl < 15))
    {
      picfwVersion[sl++] = Serial2.read();        // receive from serial USB
      //    delayMicroseconds(INNERWAIT);
    }
    picfwVersion[0] = '>';
    picfwVersion[sl] = 0;
  ...
  Serial2.write("@b");   // clear SCSgate/KNXgate buffer
  Serial2.setTimeout(10); // timeout is 10mS
  delay(50);           // wait 50ms
  Serial2.flush();
      ...
      if (busid != "")
      {
        ...
        if (deviceX)
        {
          ...
          if ((devtype == 8) || (devtype == 9) || (devtype == 4))			// w6 - aggiorna tapparelle pct su pic
          {
            ...
            requestBuffer[requestLen++] = maxp.byte.LB;    // max position L
            previous_address            = device;     // device id - address
            immediateSend();
            immediateReceive('k');
            delay(100);
          }
          
          if ((devtype == 18) || (devtype == 19) || (devtype == 24))      // U8 - aggiorna tapparelle pct su pic
          {
            requestBuffer[requestLen++] = '§';
            requestBuffer[requestLen++] = 'U';
            requestBuffer[requestLen++] = '8';
            requestBuffer[requestLen++] = linesector;
            requestBuffer[requestLen++] = previous_address;   // base address (previous)
            requestBuffer[requestLen++] = devtype;    // device type
            requestBuffer[requestLen++] = device;     // move address (actual)
...
// ------------------------------------------------------------------------------------------------------
    else
    {
      if (tcpuart == 1)
      {
        ...
        while (s < buflen)
        {
          Serial2.write(tcpBuffer[s]);   // write on serial KNXgate/SCSgate
...
//      if (badCheck > 3)    // 30 secondi 
        ...
        { 
          Serial2.write('@');    // 
          Serial2.write(0xFF);   // set led lamps zero
          delay(100);           // wait 100ms
          Serial2.write('@');    // 
          Serial2.write('|');    // 
          Serial2.write('|');    // 
    ...
    if (packetSize)
    {
      ...
      /*--------------------------------------------------------------------------
          if (strcmp(packetBuffer,"@V") == 0) // query version
          {
            ...
            {
              Serial2.write(VERSION[s]);   // write on serial KNXgate/SCSgate
      ...
      else
      {
        ...
        while (s < len)
        {
          Serial2.write(udpBuffer[s]);   // write on serial KNXgate/SCSgate
//        delayMicroseconds(120);
          s++;
        }
        tcpuart = 0;
  ...
  // rx: 04 31 00 12 00 
    ...
    while (Serial2.available() && (replyLen < lmax))
    {
      while (Serial2.available() && (replyLen < lmax))
      {
        replyBuffer[replyLen] = Serial2.read();        // receive from serial USB
...
//  tcpclient.flush(); 
  ...
  {
    if ((replyBuffer[0] == 0xF6) && (replyBuffer[1] == 'y') && (replyLen == 7))   // solo se stringa=   [0xF6] [y] 32 00 12 01 xx
    {
      content = "http://";
...
//      content += tcp_remote_ip.toString();
  ...
  { //1 (replyLen == 6) && (devIx > 0) && (replyBuffer[0] == 0xF5) && (replyBuffer[1] == DEVICEREQUEST)
//  char devCall = devIx;
    devIx  = replyBuffer[2];
    ...
    { // 2
      device_BUS_id[devIx].address = 0;
      device_BUS_id[devIx].deviceType = 0;
      device_BUS_id[devIx].alexa_id = 0;
      device_BUS_id[devIx].linesector = 0;
      device_BUS_id[devIx].moveaddress = 0;
      devIx++;
      uartSemaphor = 1;
      requestBuffer[requestLen++] = '§';
      requestBuffer[requestLen++] = DEVICEREQUEST;
  ...
  if ((replyLen == 5) && (replyBuffer[0] == 0xF4))  // 'u'  
  { //1
    ...
    { //2
      if (alexaParam == 'y')      //u//
      { //3 aggiornamento posizione coverpct fauxmo   
    ...
    // ----------------uab-(address position)--- PUBBLICAZIONE STATO COVERPCT -------------------------------------------------------
      if (mqttopen == 3)    //u//
      { //3 pubblicazione posizione coverpct   
        char actionc[6];
  ...
  } //1 replyLen==4 - 5  && replyBuffer == 0xF3-F4
  ...
  if ((mqttopen == 3) && (replyBuffer[1] == 'y') && ((replyBuffer[0] == 0xF6) || (replyBuffer[0] == 0xF7)))   
  { //1 - START pubblicazione stato device        [0xF5] [y] 32 00 12 01
    char devtype;
    char action;
  ...
  // knx ridotto  [0xF6] [y] 29 0B 65 81 bb
  // knx ridotto  [0xF7] [y] 01 0F 01 81 01 bb
  ...
  //    word device = word(replyBuffer[2], replyBuffer[3]);
        ...
        {
           deva.address = replyBuffer[replyLen-1];  // base address
        }
  ...
  // knx ridotto  [0xF6] [y] 01 0F 01 81 01
  ...
  } //1 END pubblicazione stato devices
    // ================================ F I N E   C O M A N D I     K N X ===========================================
      ...
      if (requestLen > 0)
      {
        ...
        {
#ifdef UART_W_BUFFER
          Serial2.write(requestBuffer[s]);    //  scrittura SERIALE 
  #ifdef USE_TCPSERVER
          if (tcpuart == 2) 
          {
            sprintf(logCh, "%02X ", requestBuffer[s]);
...
#else
          //       Serial2.flush();
          //       Serial2.write(requestBuffer[s]);   // write on serial KNXgate/SCSgate - 90uS
...
//------------------------------------------------------
    case PICPROG_START:
      ...
      if (SPIFFS.exists(path)) 
      {
         ...
         {
           Serial2.write('@');
           Serial2.write(0x11);   // autoprogram request
...
//------------------------------------------------------
    case PICPROG_REQUEST_WAIT:
      Serial2.setTimeout(100); // timeout calcolato per 80bytes uart (tx/rx) + program time (?)
      l = Serial2.readBytes(&bLenght, 1);
      if (l == 1)
      {
          l = Serial2.readBytes(sbuf, bLenght);
          if (l == 4)
          {
...
//    Serial2.write(0x61);  // flash programming end
      //   03 10 07 61
      prog_buffer[0] = 3;     // data length
      prog_buffer[1] = 0x10;  // from
      prog_buffer[2] = 0x07;  // format
      prog_buffer[3] = 0x61;  // command
      Serial2.write(prog_buffer, 4);
      prog_address.Val += PICBUF;
      sm_picprog = PICPROG_REQUEST_OK;
	  break;
	  
    case PICPROG_FLASH_END:
//    Serial2.write(0x80);  // flash programming end
...
char SendToPIC(char bufNr)
{
   ...
   if (len > 0)
   {
     ...
     {
#ifdef UART_W_BUFFER
          Serial2.write(serObuffer[bufNr][s]);    //  scrittura SERIALE 
  #ifdef USE_TCPSERVER
          if (tcpuart == 2) 
          {
